...
public class NearestSlotAssignmentStrategy implements  SlotAssignmentStrategy{
    ...
    public Optional<ParkingSlot> assignSlot(ParkingLot parkingLot, VehicleType vehicleType) {
        ...
        // Find the nearest spot available of the selected floor
        Optional<ParkingSlot> optNearestSlot = bestFloor.getSlots().stream()
                .filter(s -> s.getAllowedVehicles() == vehicleType &&
                        s.getSlotstate() == SlotState.EMPTY)
                .min(Comparator.comparingInt(ParkingSlot::getSlotNumber));

        return optNearestSlot;
    }
}
